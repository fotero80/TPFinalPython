...
def cargar_Literatura(request):
    if (request.method == 'POST'):
        lit = Literatura(nombre_literatura=request.POST.get(nombre_literatura), autor_literatura=request.POST.get(autor_literatura), editorial_literatura=request.POST.get(editorial_literatura), anio_edicion_literatura=request.POST.get(anio_edicion_literatura), email_usuario_literatura=request.POST.get(email_usuario_literatura))
        lit.save()
    contexto = {
...
def cargar_Musica(request, nombre_artista_musica, nombre_disco_musica, anio_lanzamiento_musica, email_usuario_musica):
    mus = Musica(nombre_artista_musica=nombre_artista_musica, nombre_disco_musica=nombre_disco_musica, anio_lanzamiento_musica=anio_lanzamiento_musica, email_usuario_musica=email_usuario_musica)
    mus.save()
    contexto = {
...
def cargar_Cine(request, nombre_pelicula_cine, nombre_director_cine, anio_lanzamiento_cine,email_usuario_cine):
    edi = Cine(nombre_pelicula_cine=nombre_pelicula_cine, nombre_director_cine=nombre_director_cine, anio_lanzamiento_cine=anio_lanzamiento_cine, email_usuario_cine=email_usuario_cine)
    edi.save()
    contexto = {
...
def datos_Usuario(request):
    if (request.method == 'POST'):
        usr = Usuario(nombre_usuario=request.POST.get(nombre_usuario), apellido_usuario=request.POST.get(apellido_usuario), fecha_nacimiento_usuario=request.POST.get(fecha_nacimiento_usuario), email_usuario=request.POST.get(email_usuario))
        usr.save()
    contexto = {
        "nombre_usuario" : usr.nombre_usuario,
        "apellido_usuario" : usr.apellido_usuario,
        "fecha_nacimiento_usuario" : usr.fecha_nacimiento_usuario,
...
def datos_crear_usuario(request):
    return render(request, 'AppCoder/usercrear.html')